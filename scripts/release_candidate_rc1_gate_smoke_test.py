...
base = Path(tempfile.mkdtemp(prefix='boostora_v300rc1_'))
os.environ['BOT_TOKEN'] = 'TEST:TOKEN'
os.environ['ADMIN_IDS'] = '1'
os.environ['BOT_DATA_DIR'] = str(base)
os.environ['DB_PATH'] = str(base / f'boostora_v300rc1_{uuid.uuid4().hex}.db')
...
def seed() -> None:
    ...
    campaign_id = db.execute(
        ...
        INSERT INTO campaigns (
            ...
            10, 'RC1 market task', 'chat_join', 'https://t.me/example',
            6, 10, 60, 40,
    ...
    for entry_type, amount in [
        ...
        db.execute(
            ...
            VALUES (10, 10, ?, 'BST', ?, ?, 'completed', ?, 'rc1 gate')
            """,
    ...
    db.execute("INSERT INTO admin_notes (admin_user_id, target_user_id, related_submission_id, note) VALUES (1, 100, NULL, 'rc1 note')")
    db.execute("INSERT INTO risk_events (user_id, event_type, severity, score_delta, details) VALUES (100, 'rc1_check', 'low', 1, 'rc1 smoke')")
...
def main() -> None:
    ...
    seed()
    gate = ReleaseReadinessService.rc1_gate_summary()
    assert gate['score'] >= 80
    assert gate['state'] in {'rc1_ready', 'controlled_rc1'}
    assert len(gate['rows']) >= 7
    assert ReleaseReadinessService.rc1_release_contract()

    import app.router as router
    text = router._build_owner_release_text(1)
    assert 'v3.0.0' in text
    assert 'RC1' in text or 'rc1' in text.lower()
    assert 'Данные не меняются' in text or 'does not mutate' in text
    print('release_candidate_rc1_gate_smoke_test: OK', flush=True)
    os._exit(0)
