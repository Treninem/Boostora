...
- По умолчанию бот использует публичный канал `@Boostorachat`.
- Владелец бота (первый ID из `ADMIN_IDS`) может в админке добавлять и удалять до 10 обязательных чатов.
- Для публичных чатов можно отправлять `@username` или ссылку `t.me/...`.
- Для приватных чатов нужно отправлять `chat_id` и invite link одной строкой.
