...
from app.i18n import ROLE_ADVERTISER
...
class Database:
    ...
    def _seed_demo(self, conn: sqlite3.Connection) -> None:
        exists = conn.execute('SELECT COUNT(*) AS c FROM campaigns WHERE is_demo=1').fetchone()['c']
        if exists:
            return
        conn.execute(
            "INSERT OR IGNORE INTO users (user_id, username, first_name, locale, role, tier, is_admin) VALUES (-1, 'boostora', 'Boostora', 'ru', ?, 'verified', 1)",
            (ROLE_ADVERTISER,),
        )
        conn.execute('INSERT OR IGNORE INTO wallets (user_id) VALUES (-1)')
        rows = [
            (-1, 'Подписка на канал партнёра', 25, 'https://t.me/BoostoraBot', 30, 0, 1),
            (-1, 'Открыть Telegram-бота партнёра', 20, 'https://t.me/BoostoraBot', 30, 0, 1),
            (-1, 'Просмотреть партнёрский пост', 15, 'https://t.me/BoostoraBot', 30, 0, 1),
        ]
        conn.executemany(
    ...
    def upsert_user(self, user_id: int, username: str | None, first_name: str | None, is_admin: bool) -> None:
        with self.connect() as conn:
            conn.execute(
                '''
                INSERT INTO users (user_id, username, first_name, locale, is_admin)
                VALUES (?, ?, ?, NULL, ?)
    ...
    def set_panel_message(self, user_id: int, chat_id: int, message_id: int) -> None:
        with self.connect() as conn:
            conn.execute(
                'INSERT INTO panel_state (user_id, chat_id, message_id) VALUES (?, ?, ?) '
                'ON CONFLICT(user_id, chat_id) DO UPDATE SET message_id=excluded.message_id',
                (user_id, chat_id, message_id),
            )
            conn.commit()

    def get_panel_message(self, user_id: int, chat_id: int) -> int | None:
        with self.connect() as conn:
            row = conn.execute('SELECT message_id FROM panel_state WHERE user_id=? AND chat_id=?', (user_id, chat_id)).fetchone()
            return int(row['message_id']) if row else None

    def clear_panel_message(self, user_id: int, chat_id: int) -> None:
        with self.connect() as conn:
            conn.execute('DELETE FROM panel_state WHERE user_id=? AND chat_id=?', (user_id, chat_id))
            conn.commit()

    def topup_wallet(self, user_id: int, amount: int) -> None:
        with self.connect() as conn:
            conn.execute('UPDATE wallets SET available=available+? WHERE user_id=?', (amount, user_id))
            conn.commit()

    def list_available_tasks(self):
        with self.connect() as conn:
            return conn.execute(
                'SELECT c.* FROM campaigns c WHERE c.completed_slots < c.total_slots ORDER BY c.is_demo DESC, c.id DESC'
            ).fetchall()
    ...
    def take_task(self, campaign_id: int, user_id: int) -> bool:
        with self.connect() as conn:
            existing = conn.execute('SELECT id FROM task_claims WHERE campaign_id=? AND user_id=?', (campaign_id, user_id)).fetchone()
            if existing:
                return False
            campaign = conn.execute('SELECT * FROM campaigns WHERE id=? AND completed_slots < total_slots', (campaign_id,)).fetchone()
            if campaign is None:
                return False
            conn.execute('INSERT INTO task_claims (campaign_id, user_id, reward) VALUES (?, ?, ?)', (campaign_id, user_id, int(campaign['reward'])))
            conn.commit()
            return True
    ...
    def complete_task(self, campaign_id: int, user_id: int) -> bool:
        with self.connect() as conn:
            ...
            if claim is None:
                return False
            conn.execute("UPDATE task_claims SET status='completed' WHERE id=?", (claim['id'],))
            conn.execute('UPDATE campaigns SET completed_slots=MIN(total_slots, completed_slots+1) WHERE id=?', (campaign_id,))
            conn.execute('UPDATE wallets SET available=available+?, earned_total=earned_total+? WHERE user_id=?', (claim['reward'], claim['reward'], user_id))
            conn.execute(
    ...
    def list_user_campaigns(self, owner_user_id: int):
        with self.connect() as conn:
            return conn.execute('SELECT * FROM campaigns WHERE owner_user_id=? ORDER BY id DESC', (owner_user_id,)).fetchall()

    def create_demo_campaign_for_user(self, owner_user_id: int) -> None:
        with self.connect() as conn:
            idx = conn.execute('SELECT COUNT(*) AS c FROM campaigns WHERE owner_user_id=?', (owner_user_id,)).fetchone()['c']
            conn.execute(
                'INSERT INTO campaigns (owner_user_id, title, reward, target_url, total_slots, completed_slots, is_demo) VALUES (?, ?, ?, ?, ?, 0, 0)',
                (owner_user_id, f'Boostora campaign #{idx + 1}', 30, 'https://t.me/BoostoraBot', 25),
            )
            conn.commit()
