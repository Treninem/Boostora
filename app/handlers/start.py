...
from app.services.payments import SPARKS_PACKS, VIP_STARS_PLANS, parse_payload
...
def register_start_handlers(bot: telebot.TeleBot) -> None:
    ...
    def handle_fallback(message: Message) -> None:
        ...
        if mode.startswith('campaign_'):
            _try_delete_user_message(bot, message)
            if mode == MODE_TARGET:
                ok, result_key, _ = ClientCampaignService.consume_target(message.from_user.id, message.text or '')
                if ok:
