...
from app.services.demo_campaigns import DemoCampaignService
...
class PerformerService:
        ...
        def _run(connection: sqlite3.Connection) -> int:
            ...
            for hold in rows:
                ...
                connection.execute(
                    ...
                    ) VALUES (?, ?, ?, 'XTR', 'credit', 'hold_release', 'completed', ?, ?, ?)
                    ''',
                    (
                        target_user_id,
                        target_user_id,
                        ...
                        hold['submission_id'],
                        hold_id,
                        'Hold released to available balance',
                    ),
                )
                released += 1
            return released
    ...
    def list_available_tasks(user_id: int, limit: int = 10):
        DemoCampaignService.ensure_demo_campaigns()
        return db.fetch_all(
            '''
            SELECT c.*
            FROM campaigns c
    ...
    def take_task(user_id: int, campaign_id: int) -> tuple[bool, str, int | None]:
        def _run(connection: sqlite3.Connection) -> tuple[bool, str, int | None]:
            ...
            if int(campaign['owner_user_id']) == user_id:
                return False, 'task_not_available', None
            if int(campaign['completed_quantity']) >= int(campaign['total_quantity']):
                return False, 'task_not_available', None
            ...
            connection.execute(
                ...
                WHERE id = ?
                ''',
                (int(campaign['reward_amount']), campaign_id),
            )
    ...
    def submit_proof(user_id: int, submission_id: int, proof_text: str) -> tuple[bool, str, int | None]:
        ...
        def _run(connection: sqlite3.Connection) -> tuple[bool, str, int | None, int]:
            ...
            connection.execute(
                ...
                WHERE id = ?
                ''',
                (reward_amount, reward_amount, reward_amount, int(campaign['id'])),
            )
            connection.execute(
                '''
                INSERT INTO transactions (
                ...
                (
                    ...
                    submission_id,
                    hold_id,
                    'Reward placed into hold after task submission',
                ),
            )
